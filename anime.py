...
def arrangeTorrent(torrentDir, torrentName):
    ...
    if os.path.isfile(torrentPath):
        ...
        reordered_path = os.path.join(checkDir(os.path.join(torrentDir,'Season ' + seasonID)),filename)
        shutil.move(torrentPath, reordered_path)
        if  os.path.isfile('./config.ini'):
            config.read('./config.ini')
            mode = config['General']['ArchiveMode']
            if mode == '1':
                targetDir = os.path.join(config['General']['LibraryPath'], originalTitle) 
                shutil.move(reordered_path, checkDir(targetDir))
